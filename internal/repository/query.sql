...
ORDER BY name LIMIT sqlc.arg('page_size')::INTEGER
OFFSET sqlc.arg('page_size')::INTEGER * (sqlc.arg('page')::INTEGER);

-- name: GetOrCreateSubgroupByName :one
INSERT INTO subgroups (name)
VALUES (sqlc.arg('name')) ON CONFLICT (name) DO
...
WHERE id = sqlc.arg('id') RETURNING id, name;
...
VALUES (sqlc.arg('name'), sqlc.arg('date_start'), sqlc.arg('date_end')) ON CONFLICT (name) DO
UPDATE SET name = EXCLUDED.name
    RETURNING id;
...
WHERE id = sqlc.arg('id') RETURNING id, name, date_start, date_end;
...
VALUES (sqlc.arg('subject_id'), sqlc.arg('category'), sqlc.arg('day'), sqlc.arg('time_start'),
        sqlc.arg('time_end'), sqlc.arg('repeat_rule'), sqlc.arg('timetable_id')) RETURNING id;
...
WHERE id = sqlc.arg('lesson_id') RETURNING id;

-- name: AssignSubgroupToLesson :exec
INSERT INTO subgroups_assignments (lesson_id, subgroup_id)
VALUES (sqlc.arg('lesson_id'), sqlc.arg('subgroup_id')) ON CONFLICT (lesson_id, subgroup_id) DO NOTHING;
...
VALUES (sqlc.arg('lesson_id'), sqlc.arg('teacher_id'),
        sqlc.arg('location_id')) ON CONFLICT (lesson_id, teacher_id, location_id) DO NOTHING;
...
FROM teacher_location_assignments WHERE lesson_id = sqlc.arg('lesson_id');
...
FROM subgroups_assignments WHERE lesson_id = sqlc.arg('lesson_id');