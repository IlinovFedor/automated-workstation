...
INSERT INTO subgroups_staging (name)
VALUES (@name);

-- name: InsertStagingTeachers :copyfrom
INSERT INTO teachers_staging (name)
VALUES (@name);

-- name: InsertStagingLocations :copyfrom
INSERT INTO locations_staging (name)
VALUES (@name);

-- name: InsertStagingSubjects :copyfrom
INSERT INTO subjects_staging (name)
VALUES (@name);

-- name: InsertStagingTimetables :copyfrom
INSERT INTO timetables_staging (name)
VALUES (@name);
...
FROM lessons l
         ...
         JOIN timetables tt ON tt.id = timetable_id
WHERE l.id IN
      (SELECT lesson_id FROM teacher_location_assignments WHERE teacher_location_assignments.teacher_id = @teacher_id)
ORDER BY tt.date_start, l.day, l.time_start, l.repeat_rule;
...
FROM lessons l
         ...
         JOIN timetables tt ON tt.id = timetable_id
WHERE l.id IN
      (SELECT lesson_id FROM teacher_location_assignments WHERE teacher_location_assignments.location_id = @location_id)
ORDER BY tt.date_start, l.day, l.time_start, l.repeat_rule;
...
SELECT CEILING(COUNT(*) / (@page_size::INT)::FLOAT) ::INT
FROM locations
WHERE (sqlc.narg(name)::TEXT IS NULL OR name ILIKE '%' || sqlc.narg(name)::TEXT || '%');
